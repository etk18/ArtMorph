export default function AuthLayout({
  ...
  return (
    <main className="grid min-h-screen grid-cols-1 lg:grid-cols-[1.1fr_0.9fr]">
      <section className="relative hidden flex-col justify-between bg-ink-950 px-12 py-16 text-white lg:flex">
        <div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-ink-800 bg-ink-900/60 p-5 text-sm text-ink-400 italic">
            &ldquo;The fastest way to match a storyboard to a new visual language.&rdquo;
          </div>
          <p className="text-xs text-ink-600">
            Crafted by <span className="text-ink-400">Eesh Sagar Singh</span>
          </p>
        </div>
      </section>
      <section className="flex items-center justify-center px-4 sm:px-6 py-10 sm:py-16">
        <div className="w-full max-w-md">
          ...
          <div className="mb-6 text-center lg:hidden">
            <p className="brand-logo">ArtMorph</p>
            <p className="mt-1 text-xs text-[var(--muted)]">Studio</p>
          </div>
          {children}
        </div>
      </section>
